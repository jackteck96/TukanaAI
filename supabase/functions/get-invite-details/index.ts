...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Para convites de cliente, buscar processo vinculado (se houver)
    if (invite.process_id) {
      ...
      // Convite de cliente sem processo vinculado
      console.log('[get-invite-details] Convite de cliente sem processo vinculado');
    }

    // 3) Buscar empresa solicitante
    const companyId = process?.company_id || invite.company_id;
    const { data: companyData, error: companyError } = await supabase
      .from('companies')
      .select('*')
      .eq('id', companyId)
    ...
    // 4) Buscar documentos solicitados (apenas se houver processo)
    if (invite.process_id) {
